(ns tech.libs.poi-test
  (:require [tech.libs.poi.parse :as poi-parse]
...
(deftest happy-path-parse-test
  (let [ds (first (poi-parse/workbook->datasets xls-file))
        ds2 (first (poi-parse/workbook->datasets xlsx-file))]
...
(deftest sparse-file-parse-test
  (let [ds (first (poi-parse/workbook->datasets sparse-file))]
